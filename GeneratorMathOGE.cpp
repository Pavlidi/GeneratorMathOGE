...
int main()
{
    setlocale(LC_ALL, "ru");
    srand(time(NULL));
    int count_task[55];              // Создает массив из номеров заданий
    for (int i = 0; i < 55; i++)     // Обнуляет количество заданий в номерах
        count_task[i] = 0;
    int choice = 0;
    ...
    do {
        ...
        case 1:
            do {
                ...
                cout << "13. Первые пять задач (" << count_task[49] + count_task[50] + count_task[51] + count_task[52] + count_task[53] + count_task[54] << ")" << endl;
